<template>
  <div>
    <div class="search">
      <el-input placeholder="Please entre account" style="width: 200px" v-model="username"></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 10,  // the number of data that can be presented in the every page
      total: 0,
      username: null,
      fromVisible: false,
      form: {},
  ...
  methods: {
    ...
    save() {   // 'save' button, it can add or update data
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            ...
            if (res.code === '200') {  // save successfully
              debugger
              this.$message.success('Save Successfully')
              this.load(1)
              this.fromVisible = false
            } else {
              this.$message.error(res.msg)  // display error massage
            }
          })
        }
      })
    ...
    del(id) {   // single delete
      this.$confirm('Are you sure about deleting it?', 'Confirm Delete', {type: "warning", confirmButtonText:"Confirm"}).then(response => {
        this.$request.delete('/admin/delete/' + id).then(res => {
          if (res.code === '200') {   // operate successfully
    ...
    handleSelectionChange(rows) {   // all rows data be currently selected
      this.ids = rows.map(v => v.id)
    },
    delBatch() {   // delete in batch
      if (!this.ids.length) {
        this.$message.warning('Please choose data')
        return
      }
      this.$confirm('Are you sure about deleting them/it?', 'Confirm Delete', {type: "warning",confirmButtonText:"Confirm"}).then(response => {
    ...
    handleAvatarSuccess(response, file, fileList) {
      // Replace the avatar attribute with a link to the uploaded image
